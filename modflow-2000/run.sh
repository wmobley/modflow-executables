...
function parse_args() {
	...
	#   $1 = mf2000DefaultDir  (baseline directory path, existing)
	#   $2 = mf2000ArchiveUrl  (optional https URL to a model zip/7z, new)
...
function fetch_archive_from_url() {
	...
	local dest="$SCRATCH_DIR/mf2000_archive_download"
	...
	if [[ ! "$url" =~ ^https://[A-Za-z0-9.-]+(/.*)?$ ]]; then
		log "ERROR: mf2000ArchiveUrl rejected — only https:// URLs are supported ($url)"
		return 1
	fi
