...
function parse_args() {
	...
	#   $1 = mf96DefaultDir  (baseline directory path, existing)
	#   $2 = mf96ArchiveUrl  (optional https URL to a model zip/7z, new)
...
function fetch_archive_from_url() {
	...
	local dest="$SCRATCH_DIR/mf96_archive_download"
	...
	if [[ ! "$url" =~ ^https://[A-Za-z0-9.-]+(/.*)?$ ]]; then
		log "ERROR: mf96ArchiveUrl rejected — only https:// URLs are supported ($url)"
		return 1
	fi
